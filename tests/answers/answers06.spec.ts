...
for (const data of test_data) {
  ...
  test(`Loan application for ${data.amount} with down payment of ${data.downPayment} is approved: ${data.expectedResult}`, async ({ page }) => {
    ...
    await page.route(`*/**/requestLoan?customerId=12212&amount=${data.amount}&downPayment=${data.downPayment}&fromAccountId=${data.fromAccountId}`, async route => {
      const mockResponse = {
        responseDate: 1758177294806,
        loanProviderName: 'Mock loan provider',
        approved: data.expectedResult,
     ...
     */
    await expect(page.locator('td[id=loanProviderName]')).toHaveText('Mock loan provider');
  });
}